...
import { percentToPx, pxToPercent, rayToPolygonDistance, remToPx, sampleAngles } from "../halo/utility"

export const randomHalo = ({ width, height }: DOMRect, { bite, rotationRange }: GeneratorOptions) => {
    const internal = () => {
        ...
        const base = ([
            ...
            [100, 100],
            [0, 100],
        ] as Point[]).map(p => percentToPx(p, width, height))
        ...
        for (const theta of angles) {
            ...
            const shrink = Math.random() < 0.5
                ? 1
                : 1 - Math.random() * (bitePx / dist)

            const finalDist = dist * shrink

            result.push([
                center[0] + dir[0] * finalDist,
                center[1] + dir[1] * finalDist,
            ])
        }
