...
export const Card = ({ borderSize = 1, children }: PropsWithChildren<CardProps>) => {
    ...
    useEffect(() => {
        setTimeout(() => {
            cardContainer.current?.classList.add("show")
        }, 2000)
    ...
    return <div className="cardContainer" ref={cardContainer}>
        <div className="contentContainer">
            {width > 0 && <svg viewBox={`0 0 ${width} ${height}`} className="hidden">
                <g fill="none" stroke="currentColor" strokeWidth={borderSize}>
                    <path d={`M0.5 ${height - 20} L0.5 0.5 L${width - 20} 0.5`}/>
                    <path d={`M${width - 1} 20 L${width - 1} ${height - 1} L20 ${height - 1}`}/>
                </g>
            </svg>}
            {children}
        </div>
